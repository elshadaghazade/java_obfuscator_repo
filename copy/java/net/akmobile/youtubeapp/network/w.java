...
/** * This class is designed for parsing the acquired data * it checks if the server sent proper response and then parses it */
public class w {
MainActivity context;
public w(MainActivity context) {
this.context = context;
}
/** * This method is created for handling the raw response from web server (in string format) * it makes json out if it and then parses this data. * @param rawResponse is the string which we got from server * @return SearchResultAdapter which handles the recycler view in SearchFragment */
public ArrayList<SearchedItem> handleVideos(String rawResponse) {
ArrayList<SearchedItem> videos = new ArrayList<>();
try {
JSONObject response = new JSONObject(rawResponse);
String status = response.getString("status");
if (!status.equals("ok")) Toast.makeText(context, response.getString("message"), Toast.LENGTH_SHORT).show();
...
JSONObject response = new JSONObject(rawResponse);
String status = response.getString("status");
if (!status.equals("ok")) Toast.makeText(context, context.getResources().getString(R.string.server_error), Toast.LENGTH_SHORT).show();
